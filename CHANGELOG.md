...
### Changed
- **Catching fire now deals its 1d6 however the fire was lit.** The opening burst was previously exclusive to `@Burning` and the API; applying the burning condition by hand — token HUD, sheet — set the creature alight but cost it nothing until its next turn, and a buff supplying `Burning` did the same. Every path that turns the condition on now ignites, including conditions arriving from another module.
  - **A buff ignites with its own formula**, so a fire built fiercer than the standard one hits as hard on ignition as it does each round afterwards. This is per switch-on: toggling a burning buff off and back on sets the creature alight again.
  - **Fire-immune creatures take nothing and keep the marker.** Unlike `apply()`, which declines to light them at all, by this point the condition is already on — and a buff-supplied one can't be removed from here regardless, since PF1 can only clear condition effects living directly on the actor. The turn-start tick already clears burning off immune creatures, so the marker resolves itself.
